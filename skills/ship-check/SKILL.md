...
- **Design consistency (any change that touched templates/CSS/components)** — compare the changed UI against repo-root `ui-registry.md` (if present): do backgrounds, borders, radii, text classes, and spacing match the registered patterns? Then run the anti-slop check on the changed surfaces: side-stripe accent borders, gradient text, glassmorphism-by-default, identical icon+heading+text card grids, an eyebrow/kicker on every section, 01/02/03 section markers as scaffolding, cream/beige body background, 24px+ card radii, em-dashes in visible copy, placeholder AI copy ("Elevate", "Seamless", "Acme"). If the `impeccable` skill is installed, prefer its detector (`detect.mjs`) for this; findings land in Layer 2 as design-system drift.
