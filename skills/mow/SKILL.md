...
| **go** fix round (closing gate findings) | **Mutation-check every guard written in the same pass as the fix it protects** — separately from the fix's own green run. A guard authored alongside its fix inherits that fix's blind spot: both come from one mental model, so the guard asserts on the shape the author already had in mind, which is the shape where the bug cannot appear. A round dispatched to close nine unfailable guards closed them and introduced three more (L50). Mutate **in memory** (`pytest -p <plugin>` at `pytest_configure`), never on disk in a tree others read (L51). **A mutant the guard's own test can mask is not a test of the guard**: when the test seizes control of the thing you mutate (its own scripted clock, its own fixture), its patch applies last and the mutant never takes effect, so mutate the code path the test does not control — a reviewer's frozen-clock mutant survived a scripted-clock test and read as a live gap until the wrapper's arithmetic was mutated instead (L66). **And a set of mutants that all die still only covers the mechanisms you imagined**: when a test asserts *how* an outcome is reached — one write, one query, no retry — assert on a count from the authoritative source rather than a substring of the statement you expect, and mutate by a mechanism your implementation does not use. A test named "single write" filtered captured SQL on `SET "password"` appearing first and was green against a queryset `.update(is_active=…, password=…)`, whose columns follow kwargs order; the two-write mutant that shared the implementation's own `save(update_fields=…)` died, so the sweep read as complete (L68) | lane / orchestrator |
| **go** same finding, third failed fix round | After **three orchestrator-dispatched** fix attempts on the **same** wave-gate or Integrate finding (fix-and-re-review still failing), **do not** dispatch a fourth fix lane for that finding. Invoke **`recover` R-06** (analysis only: spec vs plan vs code; wait for operator). Other findings keep separate counters; a lane's internal **tdd** red–green cycles are not rounds. | orchestrator |
| **go** mid-build plan drift | **`recover` R-07** — deviation list + stop for operator; no silent `checkout` / `reset` / `revert` on drifted paths | orchestrator |
...
3. **Triage findings:** Critical → fix now (new lane or foreground) and re-review the fix before the next wave — unless this is already the **third failed fix-and-re-review on the same finding**, in which case invoke **`recover` R-06** (no fourth patch lane). Warning/Suggestion → file to the tracker (taskman: tasks tagged `review-finding`, severity in title); they queue, they don't block. File from the main checkout, never from inside a lane or the integration worktree — board writes there fork `next_ids` (L49).
3a. **A failure is attributed to a lane by run counts, never by one run each side (L76).** Before you
   name a lane as the cause of a suite failure — or clear it of one — say how many runs you have on
   each side. A run with the lane's changes and a run without them are *one observation each*, and for
   an intermittent failure they carry no signal at all: the with-side failing and the without-side
   passing is exactly what a coin does. Re-run the side that failed before the claim leaves the
   session, and record the per-side counts in the finding's capture, so the next reader can see the
   evidence rather than the verdict. A real case: a full run with a new guard file failed one test and
   a run without it was green, which read as decisive; the repeat with the file included was green
   (4788 passed, exit 0), the lane was innocent, and the flake belonged to a peer stem's commit. This
   also decides where the finding is filed — attributing a peer's flake to your own lane buries it.
