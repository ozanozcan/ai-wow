---
name: imprint
description: After building any UI component, extract the visual patterns that matter for consistency and save them to ui-registry.md. Auto-invoked after impeccable and by tdd-builder / mow / project ui-designer UI lanes before Verification is complete. So every component built after this one matches what came before.
---
...
**Auto-trigger:** after any `impeccable` run that changed UI, after project `ui-designer` builds, and at the end of any mow/`tdd-builder` lane that touched templates/CSS/components — do not wait for a manual `/imprint`. Manual `/imprint` and `/imprint audit` still work.

**Where to write:** repo-root `ui-registry.md` (create if missing). On stacks without a component library (e.g. Django templates + Tailwind tokens), capture template-level patterns (banner, form, list row, empty state) and point Pattern notes at the token file (e.g. `theme/static_src/src/styles.css`) rather than inventing a second token system.
