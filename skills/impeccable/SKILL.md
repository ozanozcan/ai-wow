...
**Stack note:** impeccable is framework-agnostic. On Django SSR apps, produce Django templates + Tailwind + htmx + vanilla JS — do **not** emit React/Next/shadcn. Prefer the project-scoped `ui-designer` agent (when present under `.claude/agents/` or `.cursor/agents/`) as the stack authority; use this skill for visual craft on top of that stack.
