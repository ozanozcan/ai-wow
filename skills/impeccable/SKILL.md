---
name: impeccable
description: Frontend UI design, redesign, audit, and polish — pages, dashboards, components, mobile. UX, typography, spacing, motion, accessibility. Works on server-rendered templates (Django/Rails) as well as SPA stacks. Trigger terms — design, redesign, style, polish, page, screen, template, component, layout, navigation, form, empty state, spacing, typography, color, mobile, responsive, "looks off", "make it look good". Not for backend-only work.
version: 3.8.0
---

Designs and iterates production-grade frontend interfaces. Real working code, committed design choices, exceptional craft.

## After you ship UI (mandatory)

When this skill's work changed templates, CSS, or components — **immediately run `imprint`** (Skill tool or `/imprint` on the touched files) before declaring the UI done. Imprint appends/updates repo-root `ui-registry.md` so the next session matches what you just built. Skipping imprint leaves the registry stale; mow/`tdd-builder` treat a UI lane without imprint as incomplete.

**Stack note:** impeccable is framework-agnostic. On FitnessManager and similar Django SSR apps, produce Django templates + Tailwind + htmx + vanilla JS — do **not** emit React/Next/shadcn. Prefer the project-scoped `ui-designer` agent (when present under `.claude/agents/` or `.cursor/agents/`) as the stack authority; use this skill for visual craft on top of that stack.
