...
6. **Worktree hygiene, if `.claude/worktrees/` exists.** mow's fail-closed teardown rule (`skills/mow/SKILL.md` §Parallel lanes) says never remove a worktree with unmerged work, but nothing prompts removal once work *has* landed — this is that prompt, run every wrap-up regardless of what this chat touched. For each entry: diff it against the target branch from their merge-base; for every file that still differs (skip `docs/`), check whether that exact blob (`git hash-object`) appears anywhere in the target branch's history for that path, not just at the tip — a file that looks different at the tip because the branch kept evolving it past the worktree's snapshot is landed, not divergent. A worktree where every differing file landed somewhere (or nothing differs at all) is a removal candidate; note its last-activity date. Never infer this from file size or `--numstat` alone. Separately, check whether the worktree holds `docs/` files (verification reports, QA screenshots) absent from the target branch entirely — flag those for salvage before removal, they do not block it. Skip any worktree with a live process cwd'd inside it. Report the candidates and the exact `git worktree remove --force` + `git branch -D` commands; never run them without the operator's go-ahead, same as step 6's commit offer.
