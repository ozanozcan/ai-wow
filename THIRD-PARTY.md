...
[`LICENSE`](LICENSE). Three of the seventeen bundled skills are not — they were
...
The fourteen remaining skills — `adversarial-tester`, `bs`, `checkpoint`,
`complexity-audit`, `docs`, `imprint`, `improve-codebase-architecture`, `mow`,
`parallel-debug`, `pick-up-where-i-left-off`, `recover`, `ship-check`, `test-coverage`,
